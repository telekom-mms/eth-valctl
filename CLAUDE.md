  ...
  service/
    ...
    domain/
      ...
      request/
        ...
        request-fee-amount.ts        Parses fee amounts with units (1wei / 0.5gwei / 0.01eth)
...
| `--max-request-fee-wait-blocks <blocks>` | Max blocks to wait for the request fee to drop (`0` aborts immediately). No short alias | `50` |
| `-y, --yes` | Skip confirmation prompts by choosing safe default actions | `false` |
...
**`fees` options** (read-only; estimates and projects fees, sends nothing):
...
| `-m, --max-requests-per-block <number>` | Batch size used for the projection | global value |
| `-x, --max-request-fee <amount>` | Cap used to flag batches as exceeding | global value |
...
12. **Cap aborts throw, they do not fail a single request** — `RequestFeeCapExceededError` (exit 1) and `RequestFeeOperationCancelledError` (exit 0) propagate past per-transaction error handling via `isRequestFeePolicyStopError`, stopping the whole run
