...
describe('CLI entrypoint', () => {
  describe('global request fee options', () => {
    ...
    it('lists --max-request-fee-wait-blocks and global --yes in help', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('--max-request-fee-wait-blocks <blocks>');
  ...
  describe('fees command', () => {
    ...
    it('accepts total request count and max request fee aliases', async () => {
      const result = await runCli(['fees', 'consolidate', '--help']);

      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('-c, --total-request-count <count>');
      expect(result.stdout).toContain('-m, --max-requests-per-block <number>');
      expect(result.stdout).toContain('-x, --max-request-fee <amount>');
    });
  });
