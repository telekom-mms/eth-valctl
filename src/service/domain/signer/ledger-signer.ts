...
export class LedgerSigner implements ISigner {
  ...
  /**
   * Sign and send a transaction using the next available nonce
   *
   * Captures the nonce before awaiting to prevent concurrent reads of the same value.
   * Only increments on success so that a user rejection (which never reaches the network)
   * does not consume the nonce, keeping it available for the next attempt.
   *
  ...
  async sendTransaction(
    ...
    const response = await this.signAndSend(tx, nonce, context);
    this.nonce++;
    return response;
  }
