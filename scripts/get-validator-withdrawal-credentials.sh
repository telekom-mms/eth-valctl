...
usage() {
    cat <<EOF
This tool is for fetching current Ethereum validator withdrawal credentials on a kurtosis local devnet
...
for i in $(seq "$VALIDATOR_START_INDEX" "$VALIDATOR_STOP_INDEX"); do
    validator_withdrawal_credentials=$(curl -s "$BEACON_NODE_URL"/eth/v1/beacon/states/head/validators/"$i" | jq -r .data.validator.withdrawal_credentials)
    echo "$i: $validator_withdrawal_credentials"
done
