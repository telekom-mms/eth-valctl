...
exit_fork_version="$(curl -s "$BEACON_NODE_URL"/eth/v1/beacon/states/head/fork | jq -r .data.current_version)"
devnet_chain_settings='{"network_name": "kurtosis", "genesis_fork_version": "0x10000038", "exit_fork_version": "'"$exit_fork_version"'", "genesis_validator_root": "'"$genesis_validator_root"'"}'
